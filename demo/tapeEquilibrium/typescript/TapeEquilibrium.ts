//
//  TapeEquilibrium.ts (TypeScript, generated by: atom-typescript)
//  https://codility.com/public-report-detail/
//  https://codility.com/c/run/demoBTZCEX-QFK
//  https://codility.com/demo/results/demoBTZCEX-QFK/
//
//  Created by Isidoro Ghezzi on 4 July 2015.
//  Copyright (c) 2015 Isidoro Ghezzi. All rights reserved.
//

class IGUtil{
  public static log (theMessage){
    console.log (theMessage);
  }

  public static getRandomInt (min, max) {
    return Math.floor(Math.random() * (max - min + 1)) + min;
  }

  public static randomArray (theLength:number):number[]{
    var ret:number[] = [];
    for (let i = 0; i < theLength; ++i){
      ret.push (IGUtil.getRandomInt (-1000, 1000));
      // ret.push (1);
    }
    // IGUtil.log (ret);
    return ret;
  }
}
class TapeEquilibrium{
  private fArray:number [];
  constructor (theArray:number[]){
    this.fArray = theArray;
  }

  public doIt = ():number =>{
    var aLeftSigma = 0;
    var aRightSigma = this.fArray.reduce(function (previousValue, currentValue){
        return previousValue + currentValue;
      }, 0);
    var aMin:number = 9007199254740991; // Number.MAX_SAFE_INTEGER;
    this.fArray.slice(0, this.fArray.length - 1).forEach (function (value, index):void{
      aLeftSigma += value;
      aRightSigma -= value;
      let aDeltaAbs:number = Math.abs (aLeftSigma - aRightSigma);
      if (aDeltaAbs < aMin){
        aMin = aDeltaAbs;
      }
      // aDeltas.push ();
    });
    return aMin;
    // IGUtil.log (aDeltas);
    // return Math.min.apply (null, aDeltas);
  }
}

var solution = function (theArray) {
    var aSolver = new TapeEquilibrium(IGUtil.randomArray (10000000));
    IGUtil.log ({solver: aSolver.doIt()});
    return 1;
};
/*
var aTest = [
  IGUtil.randomArray (100000),
  [0,1],
  [3, 1, 2, 4, 3],
  [1, 2, 7],
  [-1, 3, -4, 5, 1, -6, 2, 1]
];
*/
// console.log (solution ([0]));
